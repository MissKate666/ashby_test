...
export function useDiagram(svgRef,tipRef,data,size,setParams,condition='default'){const transformRef=useRef(d3.zoomIdentity);useEffect(()=>{if(!data||!size.width||!size.height)return;const clipId=`clip-${condition}`;const svg=d3.select(svgRef.current);svg.selectAll('*').remove();const margin={top:24,right:28,bottom:56,left:72},w=size.width-margin.left-margin.right,h=size.height-margin.top-margin.bottom;const root=svg.attr('viewBox',[0,0,size.width,size.height]).classed('ashby-svg',true);const x0=d3.scaleLog().domain(data.x_range).range([0,w]).nice();const y0=d3.scaleLog().domain(data.y_range).range([h,0]).nice();const g=root.append('g').attr('transform',`translate(${margin.left},${margin.top})`);const plot=g.append('g').attr('clip-path',`url(#${clipId})`);const defs=root.append('defs');defs.append('clipPath').attr('id',clipId).append('rect').attr('x',margin.left).attr('y',margin.top).attr('width',w).attr('height',h);const groupClips=defs.append('g').attr('data-layer','group-clips');const gx=g.append('g').attr('class','axis').attr('transform',`translate(0,${h})`),gy=g.append('g').attr('class','axis');const gridX=g.append('g').attr('class','grid').attr('transform',`translate(0,${h})`),gridY=g.append('g').attr('class','grid');g.append('text').attr('x',w/2).attr('y',h+45).attr('text-anchor','middle').attr('font-weight',800).text('Плотность, кг/м³');g.append('text').attr('transform','rotate(-90)').attr('x',-h/2).attr('y',-52).attr('text-anchor','middle').attr('font-weight',800).text(data.points?.[0]?.y===data.points?.[0]?.strength?'Прочность, МПа':'Модуль Юнга, ГПа');const group=plot.append('g'),sub=plot.append('g'),pts=plot.append('g'),lineG=plot.append('g');function render(t=transformRef.current){transformRef.current=t;const x=t.rescaleX(x0),y=t.rescaleY(y0);gx.call(d3.axisBottom(x).ticks(8,'.1~g'));gy.call(d3.axisLeft(y).ticks(8,'.1~g'));gridX.call(d3.axisBottom(x).ticks(8).tickSize(-h).tickFormat(''));gridY.call(d3.axisLeft(y).ticks(8).tickSize(-w).tickFormat(''));const groupShapes=data.groups.filter(d=>d.kind==='group');const groupIndex=new Map(groupShapes.map((d,i)=>[d.id,i]));groupClips.selectAll('clipPath').data(groupShapes,d=>d.id).join('clipPath').attr('id',d=>`group-clip-${condition}-${d.id}`).selectAll('path').data(d=>[d]).join('path').attr('d',d=>blobPath(d.polygon,x,y,9));group.selectAll('path').data(groupShapes,d=>d.id).join('path').attr('class','group-shape group-blob').attr('fill',(_,i)=>groupColor(i)).attr('fill-opacity',.3).attr('stroke',(_,i)=>groupColor(i)).attr('stroke-opacity',.86).attr('stroke-width',2.4).attr('stroke-linejoin','round').attr('stroke-linecap','round').attr('d',d=>blobPath(d.polygon,x,y,9));sub.selectAll('path').data(data.groups.filter(d=>d.kind==='subgroup'),d=>`${d.id}-${d.subgroup}`).join('path').attr('class','group-shape subgroup-blob').attr('clip-path',d=>`url(#group-clip-${condition}-${d.id})`).attr('fill',(d,i)=>subgroupColor(d,i,groupIndex.get(d.id)??i)).attr('fill-opacity',.45).attr('stroke',(d,i)=>subgroupColor(d,i,groupIndex.get(d.id)??i)).attr('stroke-opacity',.6).attr('stroke-width',1.2).attr('stroke-linejoin','round').attr('stroke-linecap','round').attr('d',d=>blobPath(d.polygon,x,y,4));pts.selectAll('circle').data(data.points,d=>d.name).join('circle').attr('class',d=>`material-point ${d.is_suitable?'':'dim'}`).attr('r',d=>d.is_suitable?5:3.5).attr('fill',(d,i)=>d.is_suitable?colorFor(d,i+31):'rgb(128, 108, 121)').attr('cx',d=>x(d.x)).attr('cy',d=>y(d.y)).on('mousemove',(ev,d)=>{const tip=tipRef.current;tip.hidden=false;tip.style.left=ev.clientX+12+'px';tip.style.top=ev.clientY+12+'px';tip.innerHTML=`<b>${d.name}</b><br>${d.group} / ${d.subgroup}<br>ρ=${d.x}, y=${d.y}`}).on('mouseleave',()=>tipRef.current.hidden=true);if(data.condition_line){lineG.selectAll('path').data([data.condition_line]).join('path').attr('class','condition-line').attr('fill','none').attr('d',d3.line().x((_,i)=>x(data.condition_line.x[i])).y(v=>y(v))(data.condition_line.y)).call(d3.drag().on('drag',(ev)=>{const domainY=y.invert(ev.y);const domainX=x.invert(Math.max(1,Math.min(w,ev.x)));setParams(p=>{const nextIntercept=Math.log10(domainY)-data.condition_line.slope*Math.log10(domainX);return p.syncLines?{...p,intercept:nextIntercept}:{...p,intercepts:{...p.intercepts,[condition]:nextIntercept}}})}))}}render();root.call(d3.zoom().scaleExtent([.5,40]).on('zoom',e=>render(e.transform)));window.__ashbyZoom={...(window.__ashbyZoom||{}),[condition]:(factor)=>root.transition().call(d3.zoom().scaleBy,factor)};return()=>{if(window.__ashbyZoom)delete window.__ashbyZoom[condition]}},[data,size.width,size.height,setParams,condition])}
